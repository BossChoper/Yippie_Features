...
nutrition_values = {
    ...
    # Protein add-ons
    "lentils": {"protein": 9, "fiber": 8, "vitamins": 3, "price": 1.00},  # Veggie
    "chia seeds": {"protein": 5, "fiber": 10, "vitamins": 2, "price": 0.75},  # Nut/Seed
    "chicken strips": {"protein": 15, "fiber": 0, "vitamins": 1, "price": 2.00},  # Meat
    # Fiber add-ons
    "broccoli": {"protein": 3, "fiber": 5, "vitamins": 15, "price": 0.90},  # Vegetable
    "flaxseeds": {"protein": 3, "fiber": 8, "vitamins": 2, "price": 0.70},  # Nut/Seed
    # Vitamin add-ons
    "spinach": {"protein": 3, "fiber": 2, "vitamins": 20, "price": 0.80},  # High Calcium
    "nutritional yeast": {"protein": 8, "fiber": 3, "vitamins": 25, "price": 1.20}  # High B12
}
...
def get_add_on(diet_type, nutrient_focus):
    """Prompt user for specific add-on based on diet and nutrient focus"""
    add_on_options = {
        "protein": {
            "vegan": {"veggie protein": "lentils", "nut/seed protein": "chia seeds"},
            "vegetarian": {"veggie protein": "lentils", "nut/seed protein": "chia seeds"},
            "pescetarian": {"meat protein": "fish", "veggie protein": "lentils", "nut/seed protein": "chia seeds"},
            "no-diet": {"meat protein": "chicken strips", "veggie protein": "lentils", "nut/seed protein": "chia seeds"}
        },
        "fiber": {
            "vegan": {"vegetable fiber": "broccoli", "nut/seed fiber": "flaxseeds"},
            "vegetarian": {"vegetable fiber": "broccoli", "nut/seed fiber": "flaxseeds"},
            "pescetarian": {"vegetable fiber": "broccoli", "nut/seed fiber": "flaxseeds"},
            "no-diet": {"vegetable fiber": "broccoli", "nut/seed fiber": "flaxseeds"}
        },
        "vitamins": {
            "vegan": {"calcium": "spinach", "B12": "nutritional yeast"},
            "vegetarian": {"calcium": "spinach", "B12": "nutritional yeast"},
            "pescetarian": {"calcium": "spinach", "B12": "nutritional yeast"},
            "no-diet": {"calcium": "spinach", "B12": "nutritional yeast"}
        }
    }
    
    options = add_on_options[nutrient_focus][diet_type]
    print(f"\nFor {nutrient_focus} focus, choose an add-on: {', '.join(options.keys())}")
    while True:
...
def adjust_diet(item_name, item_details, diet_type, nutrient_focus, add_on):
    """Adjust item based on diet and chosen add-on"""
    ...
    # Add the chosen nutrient-boosting ingredient
    new_ingredients.append(add_on)
    add_on_price = nutrition_values[add_on]["price"]
    price_diff += add_on_price
    changes_made.append(f"Added {add_on} for {nutrient_focus} (+${add_on_price:.2f})")
...
def display_results(diet_type, nutrient_focus, add_on):
    """Display results based on diet, nutrient focus, and chosen add-on"""
    diet_label = "Original" if diet_type == "no-diet" else diet_type.capitalize()
    print(f"{diet_label} Menu with {nutrient_focus.capitalize()} Focus (Add-on: {add_on}) (March 17, 2025)\n" + "="*50)
    
    for item_name, item_details in menu_items.items():
        result = adjust_diet(item_name, item_details, diet_type, nutrient_focus, add_on)
...
def get_user_preferences():
    """Prompt user for diet, nutrient focus, and specific add-on"""
    ...
    add_on = get_add_on(diet, focus)
    
    return diet, focus, add_on

# Run the program
if __name__ == "__main__":
    diet_type, nutrient_focus, add_on = get_user_preferences()
    display_results(diet_type, nutrient_focus, add_on)

""" Example output:
Yippee_Testing % python diet_add_meal.py 
Available diets: vegan, vegetarian, no-diet, pescatarian
Which diet would you like to follow? no-diet

Nutrient focus options: protein, fiber, vitamins
Which nutrient would you like to boost? protein

For protein focus, choose an add-on: meat protein, veggie protein, nut/seed protein
Your choice: meat protein
Original Menu with Protein Focus (Add-on: chicken strips) (March 17, 2025)
==================================================

Item: Beef Burger
Original Ingredients: beef, american cheese, lettuce, tomatoes, onions, bun
Original Ingredients: beef, american cheese, lettuce, tomatoes, onions, bun, chicken strips
Changes Made:
  - Added chicken strips for protein (+$2.00)
Original Price: $7.00
Original Price: $8.50
Price Difference: $1.50
Nutrition Comparison:
  Original - Protein: 37g, Fiber: 6.5g, Vitamins: 22mg
  Original - Protein: 52g, Fiber: 6.5g, Vitamins: 23mg
--------------------------------------------------

Item: Chicken Sandwich
Original Ingredients: chicken, mayonnaise, lettuce, tomatoes, bun
Original Ingredients: chicken, mayonnaise, lettuce, tomatoes, bun, chicken strips
Changes Made:
  - Added chicken strips for protein (+$2.00)
Original Price: $6.50
Original Price: $7.10
...
Nutrition Comparison:
  ...
  Original - Protein: 48g, Fiber: 4.5g, Vitamins: 18.5mg
--------------------------------------------------

Item: Fish Tacos
Original Ingredients: fish, lettuce, tomatoes, tartar sauce
Original Ingredients: fish, lettuce, tomatoes, tartar sauce, chicken strips
Changes Made:
  - Added chicken strips for protein (+$2.00)
Original Price: $8.00
Original Price: $6.95
...
Nutrition Comparison:
  ...
  Original - Protein: 39g, Fiber: 2.5g, Vitamins: 19mg
--------------------------------------------------
"""

# Could be used to suggest meal plans based on user needs and input
# If user has strict protein needs, alert can be received for meal
# Need database of ingredient items with associated nutritional values
